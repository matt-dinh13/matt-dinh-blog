...
export default function PortfolioClientWrapper({ projects }: PortfolioClientWrapperProps) {
  const { language } = useLanguage()

  // Filter projects that have translations in the current language
  const projectsWithTranslations = projects.map(project => ({
    ...project,
    translations: project.portfolio_project_translations.filter((t: any) => t.language_code === language)
  })).filter(project => project.translations.length > 0)
    ...
    <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      {projectsWithTranslations.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {projectsWithTranslations.map((project: any) => {
